...
export class LastFmService {
  ...
  makeUrl = (method: string): string => {
    return `https://ws.audioscrobbler.com/2.0/?api_key=c273927229fe0ba2518dfbff80098837&format=json&method=${method}`;
  };

  getArtists = (searchTerm): any => {
    return this.http.get(`${this.makeUrl(`artist.search`)}`, {
      params: { artist: searchTerm },
    });
  };
  ...
  getArtistTopAlbums = (name: string): any => {
    return this.http.get(
      `${this.makeUrl(`artist.gettopalbums`)}&artist=${name}`
    );
  };
}
